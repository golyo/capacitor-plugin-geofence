...
public class GeofencePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    public let pluginMethods: [CAPPluginMethod] = [
        ...
        CAPPluginMethod(name: "debugSnapshot", returnType: CAPPluginReturnPromise),
    ...
    @objc func snooze(_ call: CAPPluginCall) {
        ...
        if let id = getString(call, key: "id"), let duration = getInt(call, key: "duration") ?? durationFromNumber {
            engine.snooze(id: id, durationSeconds: duration)
        }
        call.resolve()
    }

    @objc func debugSnapshot(_ call: CAPPluginCall) {
        engine.debugSnapshot { payload in
            call.resolve(payload)
        }
    }
